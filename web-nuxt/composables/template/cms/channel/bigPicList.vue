            ...
            <div class="filter-box">
                <div class="filter-item" v-for="(item, idx) in data.filterConfig" :key="idx">
                    ...
                    <el-radio-group @change="onFilterChange(item.name, $event)" :model-value="props.params.filter[item.name]">
                        <el-radio v-for="(dict, index) in item.frontend_filter_dict" :key="index" :value="index" :label="dict" border></el-radio>
                    </el-radio-group>
                </div>
            </div>
        </el-card>
        ...
        <el-card class="article-card" shadow="never">
            <template #header>
                <div class="order cms-order">
                    ...
                    <span
                        ...
                        class="order-item"
                        :class="[props.params.order == 'publish_time' ? 'active' : '', props.params.order == 'publish_time' ? props.params.sort : '']"
                    >
                        <span>发布时间</span>
                        <span class="caret-wrapper"><i class="sort-caret ascending"></i><i class="sort-caret descending"></i></span>
                    </span>
            ...
            <div>
                <div v-if="props.data.list?.length">
                    <el-row :gutter="10">
                        <el-col v-for="(product, productIdx) in props.data.list" :key="productIdx" :xs="12" :sm="12" :md="6">
