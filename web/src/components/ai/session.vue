<template>
    <div class="session-box">
        <el-row class="session-row">
            <el-col class="session-list" :sm="4">
                <el-button
                ...
                <div class="session-list-scroll ba-scroll-style">
                    <div
                        v-for="(session, idx) in sessionState.sessionList"
                        :key="idx"
                        class="session-item"
                        @click="changeSession(idx)"
                        :class="session.id == sessionState.sessionInfo.activeSessionId ? 'active' : ''"
                    >
            ...
            <el-col class="session-window" :xs="24" :sm="20">
                <div class="window-title">
                    <span>{{ sessionState.sessionInfo.title }}</span>
                    <span class="window-loading">
                        <Icon v-show="sessionState.messageStatus.loading" name="el-icon-Loading" class="is-loading" />
                    </span>
                </div>
                <TransitionGroup
                    ...
                    tag="div"
                >
                    <div v-if="sessionState.aiOutputMessageKey != null" @click="onStopGenerate" class="stop-generate" key="stopGenerate">
                        停止生成
                    </div>
                    <div class="placeholder" key="placeholder"></div>
                    <div class="chat-records-grow" key="chatRecordsGrow"></div>
                    <div
                        v-for="(item, idx) in sessionState.messages"
                                ...
                                <div class="content-tags">
                                    ...
                                    <template v-if="item.kbs?.length">
                                        <el-tooltip
                                            ...
                                            :key="kbi"
                                        >
                                            <el-tag @click="router.push({ path: '/admin/ai/kbsContent', query: { id: kbi } })" class="kbs-item">
                                                {{ kb }}
                                            </el-tag>
                                        </el-tooltip>
                                    </template>
...
import { nextTick, onActivated, reactive, onDeactivated, onUnmounted } from 'vue'
import { fullUrl, getArrayKey } from '/@/utils/common'
...
const onSubmit = () => {
    ...
    const baToken = adminInfo.getToken()
    ...
    fetchEventSource(getUrl() + '/api/ai/index', {
        method: 'POST',
        headers: {
            server: '1',
            batoken: baToken,
        ...
        body: JSON.stringify({
            ...
            id: sessionState.sessionInfo.activeSessionId,
            uuid: uuid,
        }),
        ...
        onmessage(ev) {
            ...
            if (typeof data.code != 'undefined') {
                if (data.code == 0) {
                    // code为0 -> 错误消息
                    if (sessionState.aiOutputMessageKey !== null) {
                        sessionState.messages[sessionState.aiOutputMessageKey as number].content = data.msg
                    } else {
                        addMessage(
                            {
                                content: data.msg,
                    ...
                    if (data.type == 'newTitle') {
                        sessionState.sessionList[sessionState.sessionInfo.activeSessionIdx].title = data.value
                    } else if (data.type == 'messageInfo') {
                        const messageIdx = getArrayKey(sessionState.messages, 'uuid', data.uuid + (data.value.sender_type == 'ai' ? 'you' : 'me'))
                        sessionState.messages[messageIdx].tokens = data.value.tokens
                        sessionState.messages[messageIdx].kbs = data.value.kbs
                        sessionState.messages[messageIdx].kbsTable = data.value.kbsTable
                        sessionState.sessionList[sessionState.sessionInfo.activeSessionIdx].create_time = data.value.last_message_time
                    }
                    return
                } else if (data.code == 2 && sessionState.aiOutputMessageKey !== null) {
                    // 来自ChatGPT AI的回复
                    sessionState.messages[sessionState.aiOutputMessageKey as number].content = data.content
                    sessionState.messages[sessionState.aiOutputMessageKey as number].reasoning_content = data.reasoning_content
            ...
            // 来自千问AI的回复
            if (data.request_id && sessionState.aiOutputMessageKey !== null) {
                sessionState.messages[sessionState.aiOutputMessageKey as number].content = data.message || data.output.choices[0].message.content
    ...
    sessionState.unexpectedRecords += 2
    sessionState.aiOutputMessageKey = getArrayKey(sessionState.messages, 'loading', true)
}

const onStoppedGenerate = () => {
    if (sessionState.aiOutputMessageKey === null) return
    sessionState.messages[sessionState.aiOutputMessageKey as number].loading = false
    sessionState.aiOutputMessageKey = null
...
const onAddSession = () => {
    ...
    sessionOperate({
        ...
        .then((res) => {
            sessionState.sessionList.unshift(res.data.sessionInfo)
            changeSession(0)
        })
        .finally(() => {
...
const onEditSession = (idx: number) => {
    state.editTitle.id = sessionState.sessionList[idx].id
    state.editTitle.show = true
    state.editTitle.value = sessionState.sessionList[idx].title
    nextTick(() => {
...
const onSaveSessionTitle = (idx: number) => {
    if (sessionState.sessionList[idx].operateLoading) return
    sessionState.sessionList[idx].operateLoading = true
    sessionOperate({
        id: sessionState.sessionList[idx].id,
        type: 'changeTitle',
        ...
        .then(() => {
            ...
            sessionState.sessionList[idx].title = state.editTitle.value
        })
        .finally(() => {
            sessionState.sessionList[idx].operateLoading = false
        })
}
...
const onDelSession = (idx: number) => {
    ...
    sessionOperate({
        ...
        .then((res) => {
            if (sessionState.sessionList[idx].id == sessionState.sessionInfo.activeSessionId) {
