        ...
        <el-alert class="ba-table-alert" type="info" show-icon>
            <template #default>
                <div class="ba-markdown">
                    <div>刷新表格很简单和常见，本示例花式刷新表格展示了其它表格使用细节，您可以按下<code>F12</code>查看更多信息</div>
                    <div>若单刷新表格，您只需使用<code>baTable.onTableHeaderAction('refresh', { data: 'test' })</code></div>
                </div>
            </template>
        </el-alert>
...
baTable.before.onTableHeaderAction = ({ event, data }) => {
    ...
    console.info('%c-------表格数据开始刷新--------', 'color:blue')
...
baTable.after.onTableHeaderAction = ({ event }) => {
    ...
    console.info('%c-------表格数据刷新请求完成--------', 'color:blue')
    console.info('%c-------取消延迟标记--------', 'color:blue')
...
const onRefresh = () => {
    let timer = setInterval(() => {
        ...
        if (state.seconds == 0) {
            ...
            // 刷新表格，仅此一行即可完成操作，参数二为开发者自定义的事件携带数据
