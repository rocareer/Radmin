...
class Chat
{
    ...
    public function __construct(protected int $aiUserId, protected bool $stream, array $config)
    {
        ...
        // 获取输入
        $this->inputs = request()->post();

        // 调试日志初始化
        $this->debug = config('ai.debug');
        if ($this->debug) {
            $fileName      = mb_substr(filter_var($this->inputs['message'], FILTER_SANITIZE_STRING), 0, 40);
            $this->logFile = config('ai.log_dir') . date('Y-m-d') . DIRECTORY_SEPARATOR . $fileName . '.log';
     ...
     * 检查是否有中断缓存
     */
    public function checkStopCache(): void
    {
        $cache = Cache::pull($this->aiSessionCachePrefix . $this->inputs['id']);
        if ($cache) {
    ...
    public function getMessages(): array
    {
        ...
        if ($prompt) {
            ...
            if ($irrelevantDetermination && $this->inputs['ai_irrelevant_message'] && $this->config['ai_allow_users_edit_effective_kbs']) {
                $this->error($this->inputs['ai_irrelevant_message']);
            }
     ...
     * 直接向前端发送原始输出数据
     */
    public function aliQwenOutputRead(StreamInterface $body): void
    {
        $buffer = '';
        while (!$body->eof()) {
                        ...
                        if (json_last_error() == JSON_ERROR_NONE) {
                            if (isset($rowArr['output']['choices'][0]['finish_reason'])) {
                                if ($rowArr['output']['choices'][0]['finish_reason'] != 'null') {
                                    ...
                                    Cache::delete($this->aiSessionCachePrefix . $this->inputs['id']);
                                } else {
                                    // AI生成中-缓存数据以免消息丢失
                                    $cacheData = [
                                    ...
                                    Cache::tag('ai')->set($this->aiSessionCachePrefix . $this->inputs['id'], $cacheData, DateInterval::createFromDateString('1 day'));
                                }
                            }
                            $this->output($rowJson);
                        }
     ...
     */
    public function chatGptOutputRead(StreamInterface $body): void
    {
        $buffer                   = '';
        $SSEof                    = "\n\n";
        $aiOutputMessage          = '';
     ...
     * 输出
     */
    public function output(string|array $data): void
    {
