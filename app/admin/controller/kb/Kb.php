 ...
 * 知识库管理
 */
class Kb extends Backend
{
    /**
     * Kb模型对象
     * @var object
     * @phpstan-var \app\admin\model\kb\Kb
     */
    protected object $model;

    protected array|string $preExcludeFields = ['id', 'update_time', 'create_time'];

    protected array $withJoinTable = ['kbType', 'kbCategory', 'admin', 'user'];
    ...
    {
        parent::initialize();
        $this->model = new \app\admin\model\kb\Kb();
    }
    ...
    public function index():Response
    {
        ...
        $res = $this->model
            ...
            ->order($order)
            ->paginate($limit);
        $res->visible(['kbType' => ['name'], 'kbCategory' => ['name'], 'admin' => ['username'], 'user' => ['username']]);
